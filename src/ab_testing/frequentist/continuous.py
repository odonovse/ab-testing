"""
Welch's t-test on raw, unit-level rows for a continuous metric.
For now we have assumed 2-groups (A/B), with a boolean
assignment column and (in this function) a continuous metric.
...
def t_test_continuous(
    ...
    with two groups, and a continuous metric of interest. It needs the
